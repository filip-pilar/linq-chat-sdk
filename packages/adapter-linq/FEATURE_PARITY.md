...
| `POST /v3/chats/{chatId}/messages` — send to existing chat    | Standard Chat SDK mapping | `thread.post()` / `thread.reply()`                           | Partial    | Text/media/cards, replies, decorations, service selection, and effects have central validation and Chat contracts; rich links, part-index replies, and Batch `012` remain.                                                                   | P0       | Contract-verified send/reply/options; Provider-observed `send`/`cards`      |
...
| ------------------------------------------ | ---------------------------------- | ------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------- |
| Linq message-options transport             | Typed Linq chat extension          | `linqMessage(content, options)`                               | Complete   | Immutable metadata survives Chat processing. Decorations, service, and effects compile on post/reply; metadata stays out of `SentMessage`/history. Rich-link translation remains `007B`.                                                          | P0       | Contract-verified transport, identity, serialization, options                    |
