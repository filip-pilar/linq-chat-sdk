...
| ------------------------------------------------------------------ | ------------------------- | ------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------- |
| `POST /v3/attachments` — pre-upload a file                         | Internal adapter behavior | `thread.post({ files })`              | Complete   | Buffered standard `FileUpload` data uses no-retry creation, validated name/size, preparation-only cleanup, and shared errors. Deprecated attachment `status` is not polled; large-file streaming and workflow retries/cleanup require demonstrated need. | P0       | Contract-verified upload/cleanup/errors; Provider-observed smoke `send` |
...
| Media by pre-uploaded attachment/raw bytes | Standard Chat SDK mapping          | `thread.post({ files })`                                      | Complete   | Existing sends enforce 1–255-character names and 1-byte–100MB buffered uploads with preparation-only cleanup. Large-file streaming waits for demonstrated need; retries and persistence-backed media workflows are not adapter guarantees.                         | P0       | Contract-verified boundaries/cleanup; Provider-observed `send`                    |
