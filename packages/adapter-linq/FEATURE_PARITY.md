...
| `GET /v3/messages/{messageId}`                           | Internal adapter behavior | Standard message/history/refresh behavior      | Partial    | Parsing works; delivery/reaction/rich-part normalization and error/trace contracts remain incomplete.                                                                                                    | P0       | Contract-verified retrieved-message parser                                       |
...
| Delivery/read/reconciliation state         | Typed Linq chat extension          | Generic typed `onLinqEvent` + metadata                        | Partial    | Sent/delivered/read/failed correlation and failure facts are typed. Edited/reconciled semantics remain Batch `006B`; no ordering, terminal-state, retry, or workflow guarantee is inferred.                                                       | P0       | Contract-verified fixtures, malformed/unknown data, dedupe, Chat webhook route |
...
| `message.received`              | Standard Chat SDK mapping | Standard message handlers + `onLinqEvent`     | Partial      | Standard dispatch works; direction, rich parts/status, and hostile payload coverage remain incomplete.                           | P0       | Contract-verified; Provider-observed `serve`                    |
...
| `006` | Typed Linq chat extension                          | Message lifecycle               | **In progress.** `006A` is complete with typed sent/delivered/read/failed contracts; `006B` remains planned for edited/reconciled semantics without business workflows.                                                             |
