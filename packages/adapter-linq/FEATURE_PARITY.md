...
The inventory is the authoritative status matrix for adapter-owned capabilities. It also records the
known native-client and out-of-scope boundary, but the current OpenAPI has moved ahead of the last
full mechanical audit. Batch `013` owns complete operation-by-operation reconciliation; Batch `005`
must nevertheless use the current event enum and preserve unknown future events losslessly.
...
Evidence reverified **2026-08-13**: the current OpenAPI has **66 non-webhook HTTP operations** and
**44 webhook event names**. Its 112 `operationId` values comprise those 66 callable operations plus
46 webhook example operations. The installed client remains **`@linqapp/sdk@0.32.1`** and is behind
the OpenAPI for some capabilities and event types. OpenAPI capability does not imply installed SDK
support; do not bridge client gaps with hand-written endpoint wrappers. The former 57-operation /
35-event snapshot is superseded, while full row-by-row reconciliation is deferred to Batch `013`.

The OpenAPI is ahead of that installed client for capabilities including Experience discovery and
poll events. Experience discovery remains native-client-only and waits for official client support.
Agentcard is intentionally out of scope regardless of client support; do not bridge any gap with
hand-written adapter HTTP wrappers.
...
| Linq operation                                                | Disposition               | Adapter API                                                  | Status     | Limitations / completion gap                                                                                                                                                                                                                     | Priority | Test coverage                                                              |
| ------------------------------------------------------------- | ------------------------- | ------------------------------------------------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------- | -------------------------------------------------------------------------- |
| `POST /v3/chats` — create chat with initial message           | Native Linq client        | `adapter.client.chats.create()`                              | Client API | Explicit fixed-line creation remains native-client behavior. Batch `004` documents canonical-thread handoff and validates it live; do not add `adapter.createChat()` or route it through `openDM()`.                                             | —        | Public-client contract; Batch `004` live validation                        |
...
| `POST /v3/chats/{chatId}/read` — mark read                    | Typed Linq chat extension | `adapter.conversation(threadOrId).markRead()`                | Missing    | Require canonical normalization, Linq ownership validation, DM/group constraints, repeated-call/idempotence semantics, translated errors, and live evidence; do not add a flat adapter method.                                                   | P1       | None                                                                       |
...
| Replies (`reply_to`)                       | Typed Linq chat extension          | `linqMessage(content, { replyTo })`                           | Missing    | Existing canonical chats only. Verify official-SDK request placement; omitted, zero, and nonzero part indexes; invalid ID/index errors; edit compatibility; canonical returned identity; and inbound metadata beyond `raw`.                   | P1       | Raw preservation unit only; outbound/Chat SDK contracts missing    |
...
The current OpenAPI enum has 44 names, including nine poll events absent from the old 35-event
snapshot. The installed `@linqapp/sdk@0.32.1` unwrap union is not exhaustive. Batch `005` must use an
adapter-owned event map plus a lossless future-event form; these poll rows authorize generic
delivery only, not a poll workflow. Batch `013` retains the full mechanical schema reconciliation.
...
| --------------------------------------- | ------------------------- | ------------------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------- |
| Standard Webhooks verification          | Standard Chat SDK mapping | `chat.webhooks.linq(request)`                    | Complete | Official verification plus legacy compatibility; complete dual deliveries use the documented legacy path, while invalid JSON and partial-header downgrade cases remain covered. | P0       | Unit: `adapter.test.ts`; live: `serve`                           |
...
The documentation inventory is Batch `001`; it is not an implementation batch. Batch `004` is
...
| `013` | Standard mapping + internal + typed chat extension | Inventory and parity cleanup    | **Deferred.** Mechanically reconcile the current 66-operation/44-event OpenAPI inventory and remaining adapter-owned rows; native-client, recipes, and out-of-scope boundaries remain.                                              |
