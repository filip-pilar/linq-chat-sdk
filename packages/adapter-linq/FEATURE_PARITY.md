...
1. Use standard Chat SDK APIs when their semantics match Linq:
   `thread.post()`, `thread.messages`, `thread.startTyping()`, message edit/reactions,
   `chat.openDM()`, and standard message/reaction handlers.
...
| ---------------------------- | -------------------------------------------- | ------------- | ------------------------------------------------------------------------- |
| Existing-chat text/media     | Standard mapping + internal behavior         | Partial       | Idempotency, limits, structured options, contract/error tests             |
| Proactive send/chat creation | Standard mapping + internal behavior         | Missing       | Compatible `openDM()` with internal `/v3/messages` auto-selection         |
...
| Native client boundary       | Native Linq client                           | Missing       | Expose read-only `adapter.client: LinqAPIV3`; remove convenience wrappers |
...
| ------------------------------------------------------------------ | ------------------------- | ------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------- | -------- | --------------------------------------------------------------------------------- |
| `POST /v3/attachments` — pre-upload a file                         | Internal adapter behavior | `thread.post({ files })`              | Partial | Upload is internal to standard file sending; readiness, formats, cleanup, and provider error contracts are incomplete. | P0       | Unit: outbound upload cases in `adapter.test.ts`; live: `smoke-live.mjs send`     |
...
| `DELETE /v3/attachments/{attachmentId}` — delete attachment        | Native Linq client        | `adapter.client.attachments.delete()` | Missing | Attachment administration is not standard message/file behavior; official client semantics apply.                      | —        | Native-client boundary contract only                                              |

### Available number and capabilities

| Linq operation                                      | Disposition        | Adapter API                                 | Status  | Limitations / completion gap                                                                | Priority | Test coverage                        |
| --------------------------------------------------- | ------------------ | ------------------------------------------- | ------- | ------------------------------------------------------------------------------------------- | -------- | ------------------------------------ |
| `GET /v3/available_number` — choose onboarding line | Native Linq client | `adapter.client.availableNumber.retrieve()` | Missing | Account onboarding, not a chat abstraction.                                                 | —        | Native-client boundary contract only |
| `POST /v3/capability/check_imessage`                | Native Linq client | `adapter.client.capability.checkIMessage()` | Missing | Endpoint-shaped capability query; internal routing may consume it without adding a wrapper. | —        | Native-client boundary contract only |
| `POST /v3/capability/check_rcs`                     | Native Linq client | `adapter.client.capability.checkRCS()`      | Missing | Endpoint-shaped capability query; internal routing may consume it without adding a wrapper. | —        | Native-client boundary contract only |

### Chats

| Linq operation                                                | Disposition               | Adapter API                                  | Status  | Limitations / completion gap                                                                                                                                                    | Priority | Test coverage                                                                          |
| ------------------------------------------------------------- | ------------------------- | -------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------- |
| `POST /v3/chats` — create chat with initial message           | Internal adapter behavior | Standard `chat.openDM()`/proactive post path | Missing | Use internally for compatible standard DM creation. Explicit fixed-line or endpoint-shaped creation remains `adapter.client.chats.create()`; do not add `adapter.createChat()`. | P0       | None                                                                                   |
| `GET /v3/chats` — list chats                                  | Native Linq client        | `adapter.client.chats.listChats()`           | Missing | Chat SDK has no faithful chat-list contract; do not add `adapter.listChats()`.                                                                                                  | —        | Native-client boundary contract only                                                   |
...
| `POST /v3/chats/{chatId}/messages` — send to existing chat    | Standard Chat SDK mapping | `thread.post()`                              | Partial | Text/media/cards work; rich links, replies, effects, protocol selection, idempotency, limits, and error contracts remain incomplete.                                            | P0       | Unit: text/media/cards in `adapter.test.ts` and `cards.test.ts`; live: `send`, `cards` |
...
| Linq operation                          | Disposition        | Adapter API                             | Status  | Limitations / completion gap                          | Priority | Test coverage                        |
| --------------------------------------- | ------------------ | --------------------------------------- | ------- | ----------------------------------------------------- | -------- | ------------------------------------ |
| `GET /v3/contact_card` — retrieve cards | Native Linq client | `adapter.client.contactCard.retrieve()` | Missing | Configuration query, not chat behavior.               | —        | Native-client boundary contract only |
| `POST /v3/contact_card` — set up card   | Native Linq client | `adapter.client.contactCard.create()`   | Missing | Account/device configuration is not adapter behavior. | —        | Native-client boundary contract only |
| `PATCH /v3/contact_card` — update card  | Native Linq client | `adapter.client.contactCard.update()`   | Missing | Account/device configuration is not adapter behavior. | —        | Native-client boundary contract only |
...
| `POST /v3/messages` — auto-selected sender               | Internal adapter behavior | Standard `chat.openDM()`/proactive post path   | Missing | Internal auto-selection must preserve exclusions, continuation, chosen line, reuse/new-chat result, failover reason, and idempotency. | P0       | None                                                                                |
...
| `DELETE /v3/messages/{messageId}` — delete Linq record   | Native Linq client        | `adapter.client.messages.delete()`             | Missing | Storage deletion is not recipient unsend and must never back Chat SDK `SentMessage.delete()`.                                         | —        | Native-client boundary contract only                                                |
...
| `GET /v3/messages/{messageId}/thread`                    | Native Linq client        | `adapter.client.messages.listMessagesThread()` | Missing | Linq reply threads do not match Chat SDK conversation threads; avoid an endpoint-shaped wrapper.                                      | —        | Native-client boundary contract only                                                |
...
| Linq operation                                        | Disposition        | Adapter API                                 | Status  | Limitations / completion gap                               | Priority | Test coverage                        |
| ----------------------------------------------------- | ------------------ | ------------------------------------------- | ------- | ---------------------------------------------------------- | -------- | ------------------------------------ |
| `POST /v3/payment_requests`                           | Native Linq client | `adapter.client.paymentRequests.create()`   | Missing | Official client validation, idempotency, and errors apply. | —        | Native-client boundary contract only |
| `GET /v3/payment_requests`                            | Native Linq client | `adapter.client.paymentRequests.list()`     | Missing | Official client pagination applies.                        | —        | Native-client boundary contract only |
| `GET /v3/payment_requests/{paymentRequestId}`         | Native Linq client | `adapter.client.paymentRequests.retrieve()` | Missing | Official client state model applies.                       | —        | Native-client boundary contract only |
| `POST /v3/payment_requests/{paymentRequestId}/cancel` | Native Linq client | `adapter.client.paymentRequests.cancel()`   | Missing | Official client cancellation semantics apply.              | —        | Native-client boundary contract only |
...
| Linq operation                           | Disposition        | Adapter API                            | Status     | Limitations / completion gap                                            | Priority | Test coverage                        |
| ---------------------------------------- | ------------------ | -------------------------------------- | ---------- | ----------------------------------------------------------------------- | -------- | ------------------------------------ |
| `GET /v3/phone_numbers`                  | Native Linq client | `adapter.client.phoneNumbers.list()`   | Missing    | Official client schemas and errors apply.                               | —        | Native-client boundary contract only |
| `PUT /v3/phone_numbers/{phoneNumberId}`  | Native Linq client | `adapter.client.phoneNumbers.update()` | Missing    | Account configuration is not adapter behavior.                          | —        | Native-client boundary contract only |
...
| Linq operation                                      | Disposition        | Adapter API                                      | Status  | Limitations / completion gap                                 | Priority | Test coverage                        |
| --------------------------------------------------- | ------------------ | ------------------------------------------------ | ------- | ------------------------------------------------------------ | -------- | ------------------------------------ |
| `GET /v3/webhook-events`                            | Native Linq client | `adapter.client.webhookEvents.list()`            | Missing | Administrative discovery; also useful for maintaining audit. | —        | Native-client boundary contract only |
| `POST /v3/webhook-subscriptions`                    | Native Linq client | `adapter.client.webhookSubscriptions.create()`   | Missing | Official client secret/version behavior applies.             | —        | Native-client boundary contract only |
| `GET /v3/webhook-subscriptions`                     | Native Linq client | `adapter.client.webhookSubscriptions.list()`     | Missing | Official client response types apply.                        | —        | Native-client boundary contract only |
| `GET /v3/webhook-subscriptions/{subscriptionId}`    | Native Linq client | `adapter.client.webhookSubscriptions.retrieve()` | Missing | Official client errors apply.                                | —        | Native-client boundary contract only |
| `PUT /v3/webhook-subscriptions/{subscriptionId}`    | Native Linq client | `adapter.client.webhookSubscriptions.update()`   | Missing | Official client validation applies.                          | —        | Native-client boundary contract only |
| `DELETE /v3/webhook-subscriptions/{subscriptionId}` | Native Linq client | `adapter.client.webhookSubscriptions.delete()`   | Missing | Official client deletion semantics apply.                    | —        | Native-client boundary contract only |
...
| ------------------------------------------ | ------------------------- | -------------------------------------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------------------------ |
| Plain text parts                           | Standard Chat SDK mapping | `thread.post(text)`                          | Partial | Existing-chat send works; length, idempotency, and provider error contracts remain incomplete.                                | P0       | Unit + live `send`                                     |
...
| Media by public HTTPS URL                  | Standard Chat SDK mapping | `thread.post({ attachments })`               | Partial | URL download limits, formats/counts, errors, and live failure cases remain incomplete.                                        | P0       | Unit + live `send`                                     |
| Media by pre-uploaded attachment/raw bytes | Standard Chat SDK mapping | `thread.post({ files })`                     | Partial | Internal upload readiness, size, cleanup, and complete format coverage remain incomplete.                                     | P0       | Unit + live `send`                                     |
...
| Mixed and multipart messages               | Standard Chat SDK mapping | `thread.post()`                              | Partial | Text followed by media works; total-part/public-URL limits and alternating parts remain incomplete.                           | P1       | Text+media unit/live                                   |
...
| Idempotency                                | Internal adapter behavior | Standard send paths                          | Missing | Generate/reuse a key per logical send; maximum 255 characters; no separate public idempotency API unless Chat SDK gains one.  | P0       | None                                                   |
| Auto-selected sender/failover              | Internal adapter behavior | Standard `chat.openDM()`/proactive post      | Missing | Preserve exclusions, continuation, chosen line, reuse/new-chat result, previous chat, and selection reason internally.        | P0       | None                                                   |
| Fixed-line chat creation                   | Native Linq client        | `adapter.client.chats.create()`              | Missing | Exact participant-set reuse, named-group semantics, and first-send restrictions stay on the official client.                  | —        | Native-client boundary contract only                   |
...
| Delete Linq record                         | Native Linq client        | `adapter.client.messages.delete()`           | Missing | Storage deletion only, never recipient unsend.                                                                                | —        | Native-client boundary contract only                   |
...
| Experience action                          | Native Linq client        | `adapter.client.messages.create({ action })` | Missing | Experience discovery/action invocation is not a direct adapter priority.                                                      | —        | Native-client boundary contract only                   |
...
| Message limits and validation              | Internal adapter behavior | All standard/extension send paths            | Missing | Enforce text, URL, part-count, media-count/size, first-message, consecutive-text, and service limits centrally.               | P0       | Empty-message validation only                          |
...
| Versioned payload handling              | Internal adapter behavior | Standard webhook path                | Partial  | Current message/reaction types work; unexpected versions are not validated/reported.                                      | P0       | `MessageEventV2` unit fixtures         |
| Unknown future events                   | Internal adapter behavior | Fast acknowledgement + generic event | Partial  | Safe acknowledgement exists, but events disappear until generic passthrough is implemented.                               | P0       | Unit acknowledges unknown event        |
...
| Fast acknowledgement/background work    | Standard Chat SDK mapping | `WebhookOptions.waitUntil`           | Partial  | Standard handlers receive options; generic event handlers need the same timing policy.                                    | P0       | No timing/contract test                |
| Generic verified Linq event passthrough | Typed Linq chat extension | `adapter.onLinqEvent(...)`           | Missing  | Preserve and narrow every subscribed verified envelope without adding payment, call, Agentcard, or other domain behavior. | P0       | None                                   |
...
**Disposition:** Recipe. **Status:** Missing documentation; blocked on public `adapter.client`, the
general rich-link message extension, and generic `onLinqEvent` passthrough.
...
| ----- | -------------------------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `002` | Native Linq client                                 | Minimal public foundation     | Export `LinqAdapter`, expose read-only `adapter.client: LinqAPIV3`, and contract-test only that boundary.                                               |
| `003` | Standard mapping + internal behavior               | Reliable existing-chat sends  | Complete central limits, idempotency, validation, trace-aware errors, retries, cleanup, contract tests, and applicable live coverage.                   |
| `004` | Standard mapping + internal behavior               | Proactive Chat SDK send       | Implement compatible `openDM()`/proactive posting with internal auto-selection/failover and no `sendMessage()` or `createChat()` convenience wrapper.   |
