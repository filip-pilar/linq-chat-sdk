...
| `GET /v3/chats/{chatId}/messages` — list chat messages        | Standard Chat SDK mapping | `thread.messages`; `thread.allMessages` unsupported          | Partial    | Backward history preserves usable endpoint rows/cursors. Bad rows and null/malformed/unknown parts are isolated; up to ten consecutive all-malformed pages are skipped without immediately truncating Chat SDK iteration. Cursor cycles stop safely. Provider failures use shared errors, and forward pagination remains unsupported. | P0       | Contract-verified hostile pages/parts, bounded page continuity, cursor/order, errors, Chat iteration |
...
| `GET /v3/messages/{messageId}`                           | Internal adapter behavior | Standard message/history/refresh behavior      | Complete   | Edit IDs refresh; `404` and cross-chat IDs return no message while other provider failures use shared errors. Retrieved provider facts stay lossless in `Message.raw`; `updated_at` is not edit proof.                           | P0       | Contract-verified thread correlation, refresh, tombstone, hostile parts, typed/raw fidelity, errors       |
...
| Message/history pagination direction       | Standard Chat SDK mapping          | `thread.messages`; `thread.allMessages` unsupported           | Partial    | Backward pages preserve endpoint order/cursor; Chat SDK reverses each page. Bad rows/parts are isolated, and up to ten consecutive all-malformed pages can be crossed to reach later usable history. Repeated cursors stop safely. Forward is rejected; no provider ordering guarantee is inferred. | P0       | Contract-verified bounded page continuity/order/cursor/hostile input and Chat iteration |
