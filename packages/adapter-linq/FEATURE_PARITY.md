...
form rather than treating the current 45 names as an exhaustive provider universe. Real signed
webhook replay/timing evidence remains external.
...
| Generic verified Linq event passthrough | Typed Linq chat extension | `onLinqEvent()` + verified dispatch              | Complete | Registration, lossless verified dispatch, dedupe, coexistence, failure isolation, and `waitUntil` scheduling are implemented. Real signed-webhook evidence remains external.  | P0       | Unit: verified ingress, dispatch, dedupe, isolation, scheduling |
...
| `005` | Standard mapping + internal + typed chat extension | Webhook/event foundation        | **Implementation complete; live gate pending.** `005A`–`005C` provide typed/lossless callbacks, atomic dedupe, standard coexistence, isolation, fast acknowledgement, `waitUntil`, fixtures, and durable-ingress guidance.          |
