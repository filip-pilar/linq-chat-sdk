...
| Contact sharing/read/voice   | Standard mapping + typed Linq extension      | Partial       | Standard chat-wide read plus share card/presence/voice implementation   |
| Groups and location          | Typed Linq chat extension                    | Partial       | Cohesive chat APIs and lifecycle events                                 |
...
| `POST /v3/chats/{chatId}/leave` — leave group                 | Typed Linq chat extension | `adapter.conversation(threadOrId).group.leave()`             | Missing    | Group/member minimums, post-leave behavior, and participant event contracts remain unimplemented; disposable-group observation would be supplementary.                                                                                       | P2       | None                                                                        |
...
| `POST /v3/chats/{chatId}/messages` — send to existing chat    | Standard Chat SDK mapping | `thread.post()` / `thread.reply()`                           | Partial    | Text/media/cards, replies, and decorations have central validation, idempotency, shared errors, and Chat contracts; rich links, part-index replies, effects, protocol, and Batch `012` remain.                                               | P0       | Contract-verified send/reply/decorations; Provider-observed `send`/`cards`  |
| `GET /v3/chats/{chatId}/messages` — list chat messages        | Standard Chat SDK mapping | `thread.messages` / `thread.allMessages`                     | Partial    | Cursor/limit and retrieved `parts: null` fallback work. Tombstone-page integrity, malformed/null parts, direction, and rich normalization remain incomplete.                                                                                 | P0       | Contract-verified null fallback; pagination not externally observed         |
| `POST /v3/chats/{chatId}/participants` — add participant      | Typed Linq chat extension | `adapter.conversation(threadOrId).group.addParticipant()`    | Missing    | iMessage group-only constraints, limits, handle validation, and typed event contracts remain unimplemented.                                                                                                                                  | P1       | None                                                                        |
| `DELETE /v3/chats/{chatId}/participants` — remove participant | Typed Linq chat extension | `adapter.conversation(threadOrId).group.removeParticipant()` | Missing    | iMessage group-only constraints, minimum size, self-removal distinction, and typed event contracts remain unimplemented.                                                                                                                     | P1       | None                                                                        |
...
| `POST /v3/chats/{chatId}/voicememo`                           | Typed Linq chat extension | `adapter.conversation(threadOrId).sendVoiceMemo()`           | Missing    | Initially accept public HTTPS URL or existing attachment ID only. Raw bytes/upload lifecycle wait for Batch `012`; native iMessage and RCS/SMS fallback observations would be supplementary.                                                 | P1       | Contract coverage limited to ordinary audio                                 |
...
| `GET /v3/messages/{messageId}`                           | Internal adapter behavior | Standard message/history/refresh behavior      | Partial    | Edit-event IDs can refresh the current snapshot and `404` maps to no message. `updated_at` is not treated as edit proof; delivery/reaction/rich-part and error/trace contracts remain incomplete.        | P0       | Contract-verified refresh correlation, tombstone, and retrieved parser           |
...
| ------------------------------------------ | ---------------------------------- | ------------------------------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------- |
| Linq message-options transport             | Typed Linq chat extension          | `linqMessage(content, options)`                               | Complete   | Immutable metadata uses an ordinary postable and survives Chat processing. Decorations compile on post/reply; metadata stays out of `SentMessage`/history. Remaining translation is `007B`/`010B`.                                                | P0       | Contract-verified transport, identity, serialization, decorations                |
...
| Streaming                                  | Standard Chat SDK mapping          | `thread.post(asyncIterable)`                                  | Partial    | Linq has no native stream; buffer internally to one final message and define structured-chunk behavior.                                                                                                                                           | P2       | Contract-verified; no external timing observation                                |
...
| Message limits and validation              | Internal adapter behavior          | All standard/extension send paths                             | Partial    | Existing-chat inputs enforce content/media limits plus decoration ranges and overlaps before side effects; richer-part, service, and Batch `012` lifecycle rules remain.                                                                          | P0       | Focused boundary and no-side-effect unit tests                                   |
...
| `007` | Standard mapping + typed chat extension            | Non-lossy message model         | **Partial.** `007A` freezes the ordinary `linqMessage(content, options)` transport. Rich links, part-specific reply/reactions, and inbound/history fidelity remain `007B`/`007C`; provider option translation remains later work.   |
...
| `010` | Standard mapping + typed chat extension            | Protocol, formatting, effects   | **Partial.** `010A` contract-verifies deterministic text and UTF-16 standard/manual decorations before side effects. `010B` retains preferred-service and effect policy; no recipient-presentation claim is made.                   |
