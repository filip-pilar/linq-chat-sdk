...
| ---------------------------- | -------------------------------------------- | ------------- | ----------------------------------------------------------------------- |
| Existing-chat text/media     | Standard mapping + internal behavior         | Partial       | Rich options plus Batch `012` media lifecycle                           |
| Proactive send/chat creation | Native client + recipe                       | Client API    | Batch `004` recipe/contracts; no `openDM()`                             |
| Webhooks                     | Standard + internal + typed chat extension   | Complete      | Batch `006` lifecycle fidelity                                          |
| Contact sharing/read/voice   | Standard mapping + typed Linq extension      | Partial       | Frozen facade contract; share card/presence/voice implementation        |
| Groups and location          | Typed Linq chat extension                    | Partial       | Frozen nested contracts; operation and lifecycle implementation         |
...
Batch `004` is reduced to recipe documentation and compile-time contracts. It adds no runtime
...
adapter completion. A first-class proactive extension remains deferred until demonstrated usage
justifies it.
...
| ------------------------------------------------------------------ | ------------------------- | ------------------------------------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------------------------------------------------------------- |
| `POST /v3/attachments` — pre-upload a file                         | Internal adapter behavior | `thread.post({ files })`              | Partial    | Name/size validation, no-retry creation, preparation cleanup, and shared errors work; readiness, upload retries, complete formats, send-time lifecycle, and retention wait for Batch `012`. | P0       | Contract-verified upload/cleanup/errors; Provider-observed smoke `send` |
| `GET /v3/attachments/{attachmentId}` — retrieve metadata/fresh URL | Internal adapter behavior | Attachment `fetchData()`/rehydration  | Partial    | Refresh is internal to standard attachment downloads; expiry and ephemeral-tier contracts remain incomplete.                                                                                | P0       | Contract-verified inbound/rehydration; no external observation          |
...
| `GET /v3/chats/{chatId}` — retrieve chat                      | Internal adapter behavior | Standard thread construction/refresh                         | Partial    | Internal mapping works and preserves raw metadata; not-found handling remains incomplete.                                                                                                                                                    | P0       | Contract-verified thread/identity; indirect Provider-observed use           |
| `PUT /v3/chats/{chatId}` — update group name/icon             | Typed Linq chat extension | `adapter.conversation(threadOrId).group.update()`            | Missing    | Facade contract is frozen without provider I/O. Group-only behavior, public HTTPS icon validation, and typed success/failure events remain unimplemented; exact request correlation is not promised without a provider key.                  | P1       | Contract-verified facade shape and no-I/O rejection                         |
| `POST /v3/chats/{chatId}/leave` — leave group                 | Typed Linq chat extension | `adapter.conversation(threadOrId).group.leave()`             | Missing    | Facade contract is frozen without provider I/O. Group/member minimums, post-leave behavior, and participant event contracts remain unimplemented; disposable-group observation would be supplementary.                                       | P2       | Contract-verified facade shape and no-I/O rejection                         |
| `GET /v3/chats/{chatId}/location` — get shared locations      | Typed Linq chat extension | `adapter.conversation(threadOrId).location.retrieve()`       | Missing    | Facade and minimal snapshot contracts are frozen without provider I/O. GeoJSON validation, freshness, handle matching, empty results, and polling guidance remain unimplemented.                                                             | P1       | Contract-verified result shape and no-I/O rejection                         |
| `POST /v3/chats/{chatId}/location/request`                    | Typed Linq chat extension | `adapter.conversation(threadOrId).location.request()`        | Missing    | Facade contract is frozen without provider I/O. One-to-one iMessage constraints, recipient consent, and asynchronous sharing lifecycle remain unimplemented.                                                                                 | P1       | Contract-verified facade shape and no-I/O rejection                         |
| `POST /v3/chats/{chatId}/messages` — send to existing chat    | Standard Chat SDK mapping | `thread.post()` / `thread.reply()`                           | Partial    | Text/media/cards, rich links, replies, part targeting, decorations, service, and effects have central validation and Chat contracts; Batch `012` remains.                                                                                    | P0       | Contract-verified send/reply/options/parts; Provider-observed send/cards    |
| `GET /v3/chats/{chatId}/messages` — list chat messages        | Standard Chat SDK mapping | `thread.messages`; `thread.allMessages` unsupported          | Partial    | Backward history preserves usable endpoint rows/cursors. Bad rows and null/malformed/unknown parts are isolated; forward pagination is unsupported.                                                                                          | P0       | Contract-verified hostile pages/parts, cursor/order, Chat iteration         |
| `POST /v3/chats/{chatId}/participants` — add participant      | Typed Linq chat extension | `adapter.conversation(threadOrId).group.addParticipant()`    | Missing    | Facade contract is frozen without provider I/O. iMessage group-only constraints, limits, handle validation, and typed event contracts remain unimplemented.                                                                                  | P1       | Contract-verified facade shape and no-I/O rejection                         |
| `DELETE /v3/chats/{chatId}/participants` — remove participant | Typed Linq chat extension | `adapter.conversation(threadOrId).group.removeParticipant()` | Missing    | Facade contract is frozen without provider I/O. iMessage group-only constraints, minimum size, self-removal distinction, and typed event contracts remain unimplemented.                                                                     | P1       | Contract-verified facade shape and no-I/O rejection                         |
...
| `POST /v3/chats/{chatId}/share_contact_card`                  | Typed Linq chat extension | `adapter.conversation(threadOrId).shareContactCard()`        | Missing    | Facade contract is frozen without provider I/O. Sharing behavior remains unimplemented; configuration stays on `adapter.client`/dashboard.                                                                                                   | P1       | Contract-verified facade shape and no-I/O rejection                         |
| `POST /v3/chats/{chatId}/typing` — start typing               | Standard Chat SDK mapping | `thread.startTyping()`                                       | Partial    | DM start works; long-work refresh and stable error semantics remain incomplete.                                                                                                                                                              | P1       | Contract-verified typing cases; indirect provider observation               |
| `DELETE /v3/chats/{chatId}/typing` — stop typing              | Typed Linq chat extension | `adapter.conversation(threadOrId).stopTyping()`              | Missing    | Facade contract is frozen without provider I/O. Chat SDK has no stop method; implementation must document automatic clear-on-send/timeout and avoid a raw endpoint alias.                                                                    | P2       | Contract-verified facade shape and no-I/O rejection                         |
| `POST /v3/chats/{chatId}/voicememo`                           | Typed Linq chat extension | `adapter.conversation(threadOrId).sendVoiceMemo()`           | Missing    | URL/attachment-ID input and stable identity result contracts are frozen without provider I/O. Validation and sending remain unimplemented; raw bytes/upload lifecycle wait for Batch `012`.                                                  | P1       | Contract-verified input/result shape and no-I/O rejection                   |
...
| `POST /v3/messages` — auto-selected sender               | Native Linq client        | `adapter.client.messages.create()`             | Client API | Approved proactive recipe: one idempotency key per logical send, then construct a canonical Chat SDK thread from returned `chat_id`. Creation/reuse/retry/failover/concurrency remain provider behavior. | —        | Contract-verified public-client handoff; selective provider observation optional |
| `GET /v3/messages/{messageId}`                           | Internal adapter behavior | Standard message/history/refresh behavior      | Partial    | Edit IDs refresh; `404` returns no message. Retrieved provider facts stay lossless in `Message.raw`; `updated_at` is not edit proof, and provider error refinement remains.                              | P0       | Contract-verified refresh, tombstone, hostile parts, typed/raw fidelity          |
| `PATCH /v3/messages/{messageId}` — edit text part        | Standard Chat SDK mapping | `SentMessage.edit()`                           | Partial    | Part 0 works; outbound-only, time/edit-count, text-part, and part-index constraints need stable handling.                                                                                                | P1       | Contract-verified part-0 happy path; no external observation                     |
...
| `POST /v3/messages/{messageId}/reactions`                | Standard Chat SDK mapping | `SentMessage.addReaction()`/`removeReaction()` | Partial    | Standard/custom emoji work; part-specific/sticker behavior and async error handling remain incomplete.                                                                                                   | P0       | Contract-verified reactions/webhooks; Provider-observed inbound reaction         |
...
| Plain text parts                           | Standard Chat SDK mapping          | `thread.post(text)`                                           | Partial    | Existing-chat sends enforce non-empty/10,000-character limits, idempotency, SDK retry ownership, and shared provider errors. Initial proactive sends remain the Batch `004` native-client recipe; no adapter `openDM()` path is planned.          | P0       | Contract-verified; Provider-observed `send`                                      |
...
| Media by public HTTPS URL                  | Standard Chat SDK mapping          | `thread.post({ attachments })`                                | Partial    | Existing sends validate HTTPS and the 40-part sub-limit while preserving by-reference delivery; inbound download security/bounding is implemented, while complete formats and failure contracts remain incomplete.                                | P0       | Contract-verified boundaries/download; Provider-observed `send`                  |
| Media by pre-uploaded attachment/raw bytes | Standard Chat SDK mapping          | `thread.post({ files })`                                      | Partial    | Existing sends enforce 1–255-character names and 1-byte–100MB uploads with preparation cleanup; readiness, upload retries, full formats, retention, and send-time lifecycle wait for Batch `012`.                                                 | P0       | Contract-verified boundaries/cleanup; Provider-observed `send`                   |
| Inbound media and fresh downloads          | Standard Chat SDK mapping          | `message.attachments` / `fetchData()`                         | Partial    | Bounded refresh/download works; ephemeral-tier and format-alias contracts remain.                                                                                                                                                                 | P0       | Contract-verified; no external observation                                       |
...
| Native Name and Photo contact sharing      | Typed Linq chat extension          | `adapter.conversation(threadOrId).shareContactCard()`         | Missing    | The facade declaration is complete, but provider behavior remains unimplemented; configuration stays on client/dashboard.                                                                                                                         | P1       | Contract-verified facade shape and no-I/O rejection                              |
| Native voice-memo bubble                   | Typed Linq chat extension          | `adapter.conversation(threadOrId).sendVoiceMemo()`            | Missing    | Public HTTPS URL/existing attachment-ID inputs and canonical identity results are frozen; provider behavior remains unimplemented, and raw-byte/upload lifecycle waits for Batch `012`.                                                           | P1       | Contract-verified input/result shape and no-I/O rejection                        |
| Standard tapbacks                          | Standard Chat SDK mapping          | Standard reaction APIs/handlers                               | Partial    | Six tapbacks work both directions. Exact-line device checks observed a personal `like` in Linq history and an adapter `heart` as an iMessage `love`; remove/error cases remain.                                                                   | P0       | Contract-verified; Device-observed exact-line both directions                    |
...
| Edit text part                             | Standard Chat SDK mapping          | `SentMessage.edit()`                                          | Partial    | Standard part-0 edit uses deterministic plain text. SDK `0.42.0` cannot replace decorations; time/count constraints and arbitrary parts remain incomplete.                                                                                        | P1       | Contract-verified text compilation; unit happy path                              |
...
| Ephemeral message/attachment retention     | Internal adapter behavior          | Standard history/attachment behavior                          | Partial    | Refresh works; document visible retention behavior and cover ephemeral cleanup where available.                                                                                                                                                   | P2       | Contract-verified refresh; external observation only if enabled                  |
| Message limits and validation              | Internal adapter behavior          | All standard/extension send paths                             | Partial    | Existing-chat inputs enforce content/media/decorations plus contradictory explicit service intent before side effects; richer-part and Batch `012` lifecycle rules remain.                                                                        | P0       | Focused boundary and no-side-effect unit tests                                   |
...
| `message.received`              | Standard Chat SDK mapping | Standard message handlers + `onLinqEvent`         | Partial      | Live inbound standard dispatch works; recovered history is generic-only. Rich parts/status and broader hostile payload coverage remain incomplete. | P0       | Contract-verified including reconciled suppression; Provider-observed `serve` |
...
| `reaction.added`                | Standard Chat SDK mapping | Standard `onReaction()` + `onLinqEvent`           | Partial      | Standard/custom reactions work; sticker/part-specific data needs lossless extension handling.                                                      | P0       | Contract-verified; Provider-observed inbound                                  |
| `reaction.removed`              | Standard Chat SDK mapping | Standard `onReaction()` + `onLinqEvent`           | Partial      | Same limitations as added reactions.                                                                                                               | P0       | Contract-verified; Provider-observed inbound                                  |
...
| `004` | Native Linq client + recipe                        | Proactive native-client handoff | **Planned documentation/contracts only.** Keep `openDM()` unsupported; use `messages.create()`, then construct `linq:{chatId}` from the returned canonical ID. No runtime adapter extension.                                        |
...
| `008` | Typed Linq chat extension                          | Contact, presence, and voice    | **Partial.** `008A` freezes canonical ownership plus exact common, `.group`, `.location`, voice-input, and stable-result contracts without provider I/O. `008B`/`008C` remain planned implementations.                              |
| `009` | Typed Linq chat extension                          | Groups and location             | **Planned.** Add existing-group behavior under `.group`, location under `.location`, typed lifecycle events, constraints, and record any selective provider/device observations separately.                                         |
