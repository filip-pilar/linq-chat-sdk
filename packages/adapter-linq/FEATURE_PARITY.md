...
| Contact sharing/read/voice   | Standard mapping + typed Linq extension      | Partial       | Read/share/typing stop complete; native voice implementation            |
...
| `POST /v3/chats/{chatId}/typing` — start typing               | Standard Chat SDK mapping | `thread.startTyping()`                                       | Partial    | DM start works. Current Linq docs support direct/group chats, but the existing adapter still skips known groups; group alignment, long-work refresh, and stable errors remain outside `008B`.                                                  | P1       | Contract-verified existing typing cases; indirect provider observation       |
...
| `008` | Typed Linq chat extension                          | Contact, presence, and voice    | **Partial.** `008A` freezes the facade; `008B` implements typing stop/contact sharing and preserves the completed part-reaction path with exact calls, shared errors, and standard Chat coexistence. `008C` voice remains planned.  |
