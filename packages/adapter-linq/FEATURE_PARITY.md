...
| Webhooks                     | Standard + internal + typed chat extension   | Partial       | Dispatch registered generic events, dedupe, then chat lifecycle fidelity |
...
| Standard tapbacks                          | Standard Chat SDK mapping          | Standard reaction APIs/handlers                               | Partial    | Six tapbacks work both directions. Exact-line device checks observed a personal `like` in Linq history and an adapter `heart` as an iMessage `love`; remove/error cases remain.                                                               | P0       | Unit + exact-line live both directions                             |
...
Slices `005A`/`005B` implement this one/many/all registration surface, its adapter-instance
registry, the checked-in OpenAPI-derived 45-name type inventory, verified fan-out, standard
message/reaction coexistence, and atomic provider/partner/event deduplication. A claimed callback is
at-most-once attempted: handler failure is isolated and logged, not made eligible for provider
retry. Acknowledgement scheduling remains `005C`. All-event handlers retain a lossless future-event
form rather than treating the current 45 names as an exhaustive provider universe.
...
| Typed generic event registration        | Typed Linq chat extension | `adapter.onLinqEvent(one / many / all)`          | Partial  | Registration and verified callback dispatch are complete. One-step callbacks remain response-blocking until `005C` adds `waitUntil` scheduling.                               | P0       | Registry, dispatch, isolation + public compile-time contracts  |
...
| Unknown future events                   | Internal adapter behavior | Verified result + fast acknowledgement           | Partial  | Unknown current names and unsupported versions reach all-event handlers losslessly; non-blocking acknowledgement remains `005C`.                                              | P0       | Unit verifies preservation, dispatch, and acknowledgement      |
...
| Generic verified Linq event passthrough | Typed Linq chat extension | `onLinqEvent()` + verified dispatch              | Partial  | Registration, lossless verified dispatch, dedupe, coexistence, and failure isolation are complete; generic `waitUntil` scheduling remains `005C`.                             | P0       | Unit: verified ingress, dispatch, dedupe, callback isolation   |
...
| `005` | Standard mapping + internal + typed chat extension | Webhook/event foundation        | **`005A`/`005B` complete.** Typed registration, lossless verified dispatch, atomic dedupe, standard coexistence, and callback isolation are implemented; `005C` retains `waitUntil`, fixtures, guidance, and live evidence.         |
