...
Slice `005A` implements this one/many/all registration surface, its adapter-instance registry, and
the checked-in OpenAPI-derived 45-name type inventory. Registration does not yet invoke callbacks:
verified fan-out, coexistence with standard message/reaction dispatch, and deduplication remain
`005B`; acknowledgement scheduling remains `005C`. All-event handlers retain a lossless future
event form rather than treating the current 45 names as an exhaustive provider universe.
...
| Typed generic event registration        | Typed Linq chat extension | `adapter.onLinqEvent(one / many / all)`          | Partial  | `005A` registry, narrowing, unsubscribe, current-name drift, and future-event contracts are complete. Verified callback dispatch/dedupe/waitUntil remain `005B`/`005C`.       | P0       | Registry unit + public compile-time contracts                  |
...
| `005` | Standard mapping + internal + typed chat extension | Webhook/event foundation        | **`005A` complete.** Typed one/many/all registration, unsubscribe, current-name drift checking, and lossless future contracts are implemented; `005B`/`005C` retain verified dispatch, dedupe, `waitUntil`, and fixtures.           |
