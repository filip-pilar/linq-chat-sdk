...
| `009` | Typed Linq chat extension                          | Groups and location             | **Complete.** `009A` implements validated existing-group operations; `009B` adds acknowledgement-only location requests, defensive immutable retrieval snapshots, and typed/raw location-sharing observations. `009C` remains optional evidence. |
